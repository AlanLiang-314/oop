...
public:
    ...
    const int getItem(int key) const {
        // printf("touch %d->%d, locked %d\n", key, forward[key], lockReverse[key] ? 1: 0);
        return forward[key];
    }

    const int getReverseItem(int key) const {
        return reverse[key];
    }
...
/*在一張連通的無向圖中，假設圖上兩點a b 距離為 x, 有沒有可能在所有a的neighbor中，沒有一個和b的距離比 x 還小的*/

std::vector<int> findPath(const std::vector<std::vector<int>>& distances, const std::pair<int, int>& gate, const BiDict& qubitMapping, const Graph& g) {
    auto a = qubitMapping.getItem(gate.first), b = qubitMapping.getItem(gate.second);
    auto distance = distances[a][b];
    int idx = 0;
    ...
    while (a != b)
    {
        ...
        for(const auto& neighbor : neighbors) {
            if(distance - distances[b][neighbor] > 0) {
                // printf("good %d->%d\n", a, neighbor);
                a = neighbor;
                distance = distances[b][neighbor];
                path[idx++] = neighbor;
            }
        }
    }
...
std::vector<std::pair<int, std::pair<int, int>>> sabresSwap(const std::vector<std::pair<int, int>>& gates, const std::vector<std::pair<int, int>>& dependencies, const Graph& g, BiDict& qubitMapping, int logQubits, int numGates) {
    ...
    while (true) {
        ...
        // std::unordered_map<int, int> swapMap;
        std::vector<int> swapMap(logQubits, 0);

        printf("excutable size %d\n", executableQueue.size());

        for(const auto& gateId : executableQueue) {
            const auto& gate = gates[gateId];
            std::vector<int> path = findPath(allPairDistance, gate, qubitMapping, g);
        ...
        const auto& neighbors = g.getNeighbor(45);
